...
impl McpRegistry {
    ...
    pub async fn test(
        ...
        command
            ...
            .stderr(std::process::Stdio::piped());
        let mut child = command.spawn()?;
        let request = serde_json::json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"vera","version":"0.1.0-alpha.5"}}});
        if let Some(mut stdin) = child.stdin.take() {
